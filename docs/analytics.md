...
| `daily_recap_engaged` | `day_number, source, vs_ghost, hard` | daily done screen | The "Your five places" recap was actually engaged — at most once per done-screen render. `source` is `swipe` (a carousel card scrolled) \| `overview_tap` (an overview pin tapped); `vs_ghost` / `hard` tag which board. The recap's city names live in the DOM (masked), never on the event — aggregates only, no place name or coordinate rides |
...
| **Recap engagement** | Do finishers actually look back at their five places? Engagement rate = `daily_recap_engaged ÷ daily_challenge_completed`; `source` split (`swipe` vs `overview_tap`) shows which affordance carries it. A high rate on duel runs (`vs_ghost=true`) validates the owner's trigger — replaying a friend's ghost link and wanting to know what the answers were | `daily_recap_engaged`, `daily_challenge_completed` |
