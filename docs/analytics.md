...
| `team_name_used` | `mode, source` | host phone (couch setup) | Team-roster brief: fired once per named team at game creation. `source` is `typed` (hand-entered) \| `pun` (🎲 Surprise me picked a geography pun) \| `recent` (a Recent teams chip, a type-ahead pick, or the untouched last-used-name pre-fill). Never the team name itself — measures whether the pun bank / recent roster cut typing, not what anyone is called |
...
| **Team-name entry friction** (team-roster brief) | `team_name_used.source` mix (`typed` \| `pun` \| `recent`) per named couch team: does the 🎲 pun bank / Recent teams roster actually replace typing, or does everyone still hand-type? A rising `pun`+`recent` share is the feature working | `team_name_used` |
