...
| `daily_challenge_completed` | `day_number, score, rounds_played, best_distance_km, hard, vs_ghost, streak, pb, aces` | daily page | All five rounds resolved (forfeits included). `rounds_played` counts pinned rounds (0–5); `best_distance_km` is the closest guess, absent when all forfeited. G1/G4/G5/G6/G8: `hard`, `vs_ghost`, `streak` (after the run), `pb` (a personal best was set), `aces` (this run's sub-1km pins). Exhibition duels (an already-completed board replayed against a ghost) emit neither this nor `daily_challenge_started` — only counted runs report |
| `ghost_duel_completed` | `day_number, outcome, margin, hard` | daily page (recipient) | G5: fired on the recipient's device at the verdict. `outcome` is `won` \| `lost` \| `tie`; `margin` is the score difference (the same class of aggregate as `winning_score`). The bottom of the duel funnel — do duels resolve, who wins, are margins close? No pin, timing, or payload byte rides. Exhibition duels emit neither this nor `daily_challenge_started` — only counted runs report |
...
| `daily_resumed` | `day_number, rounds_done, hard, action` | daily page | Mid-run persistence (`docs/daily-persistence-spec.md`): fired at the moment of choice on a device that had saved mid-run state. `action` is `resume` (continued at round `rounds_done`+1 — includes the 5-round finalize rescue after a crash at the finish line) \| `restart` ("Start over" chosen — a real second start that also re-fires `daily_challenge_started`) \| `discarded` (the save was invalid / pool-drifted and a fresh run started). `rounds_done` is how many rounds the save held (0 when unparseable). `hard` tags the board. `daily_challenge_started` is deliberately **not** re-fired on a `resume` (it fired before the reload; the funnel stays 1 started : 1 completed), and `daily_challenge_completed` gains no property — the existing completion funnel measures the effect unchanged. No coordinate, pin, cursor, image id, or payload byte rides — aggregates only |
| `party_recap_engaged` | `room, mode, surface, rounds_shown, source` | host phone / player phone | The party game-over "Where were the places" recap was actually engaged — at most once per game-over render. `source` is `swipe` (a carousel card scrolled); `surface` is `host` \| `player` (the TV recap is passive — no interaction to measure, no event). `rounds_shown` is how many cards the carousel held (≤ `roundCount` when a device missed rounds). Aggregates only — no place name, coordinate, or team name rides (`docs/party-recap-spec.md`) |
...
| `render_probe` | `surface, round_number, verdict, ctx_lost, canary_ok, sample, since_load_ms, net_type, online` | the viewer wrapper | §18 (`docs/ios-blackout-review.md`): one per **non-alive** render-death probe verdict, capped at ≤4/round by the probe schedule (+1500ms/+5000ms after each anchor/resume load, +300ms on `visibilitychange→visible`). `verdict` is `dead` (context lost or the offscreen GPU canary failed — confirmed, may trigger the §3 rebuild) or `suspect` (blank/unreadable sample with the context *not* provably lost — **instrumented but never acted on** in v1; the measurement stream for the §2.3 widening decision). `ctx_lost` is `gl.isContextLost()` on the SDK's own context (absent when no context was obtainable); `canary_ok` is the 1×1 offscreen WebGL canary (absent when it didn't run — it's created lazily, only on a suspicious probe); `sample` (`content`\|`blank`\|`unreadable`\|`skipped`) is **corroboration only** — `content` proves paint, `blank` proves nothing (three.js `preserveDrawingBuffer:false` clears the buffer on a healthy canvas too, so `blank` can never reach `dead`). Aggregates only — no canvas pixel, image id, coordinate, or user input rides |
| `render_recovery` | `surface, round_number, attempt, trigger, result, duration_ms, net_type, online` | the viewer wrapper | §18: one per bounded in-place viewer-rebuild attempt, capped at `RENDER_REBUILD_MAX_PER_ROUND`=1 / `RENDER_REBUILD_MAX_PER_SESSION`=2 by the pure bounds in `imagery.js` (a device under persistent GPU pressure must not enter a destroy/create loop that *worsens* the pressure). `trigger` is why the canvas was condemned (`context_lost`\|`canary_dead`); `result` is `recovered` (resume ok + the re-armed probe reads alive), `rebuild_failed` (constructor or resume `moveTo` failed), or `still_dead` (resume ok but the re-armed probe condemns again — the GPU layer is simply gone right now). Its **own** event, deliberately not folded into `edge_recovery`, so the spatial-edge-recovery KPI stays uncontaminated (§5). Aggregates only |
...
| **Daily actives & retention** (S2) | The ritual KPI: `daily_challenge_started` unique users per day (daily actives); PostHog Retention on `daily_challenge_started` (does the ritual bring people back?) and on `game_created` (does the daily feed the party game?). Completion rate = `daily_challenge_completed ÷ daily_challenge_started`; both sides exclude exhibition duels (neither event fires for them), so the ratio stays honest. `score` / `best_distance_km` distributions calibrate difficulty day over day | `daily_challenge_started`, `daily_challenge_completed`, `game_created` |
| **Recap engagement** | Do finishers actually look back at their five places? Engagement rate = `daily_recap_engaged ÷ daily_challenge_completed`; `source` is `swipe` (a carousel card scrolled). A high rate on duel runs (`vs_ghost=true`) validates the owner's trigger — replaying a friend's ghost link and wanting to know what the answers were. This same swipe rate is the measure for the eager-init tweak (first two cards render up front so a second map peeks in and signals there's more to swipe) — no separate event | `daily_recap_engaged`, `daily_challenge_completed` |
| **Party recap engagement** | The party analogue of the above — do party players look back at the places? Engagement rate = `party_recap_engaged ÷ game_completed`, split by `surface` (`host` \| `player`) and `mode`. The owner personally wanted this recall aid on the party game-over, so the rate answers whether it earns its place; the TV recap is passive (auto-cycling) and deliberately carries no event — "was a TV attached at game-over" is already answerable via `screen_joined` + `game_completed` | `party_recap_engaged`, `game_completed` |
| **Resume rescue rate** (`docs/daily-persistence-spec.md`) | Does mid-run persistence move Daily completion — especially the iOS funnel the incident came from? **Rescue rate** = `daily_resumed[action=resume] ÷ daily_challenge_started` — how often the net catches a reload/lock/crash mid-run. Then the follow-through funnel `daily_resumed[action=resume] → daily_challenge_completed`, split by device type, answers whether a caught run gets finished and whether it lifted iOS completion. `action` breakdown (`resume` \| `restart` \| `discarded`) shows how often the save is usable vs. drifted vs. abandoned; `restart` also re-fires `daily_challenge_started`, so it stays honest in the actives/completion ratios above | `daily_resumed`, `daily_challenge_started`, `daily_challenge_completed` |
...
| **Render-death rate** | `render_dead` issues (the `trackError` class + `pano_session.render_dead=true`) ÷ `viewer_init ok=true`, cut by `$os`/`$browser` — the number that was **invisible** on 2026-08-28 (a black pano behind a live HUD emitted nothing). Should surface precisely in the currently-silent cohort (iOS, zero-`$exception` histories) once §18 ships | `render_dead` (`$exception`), `pano_session`, `viewer_init` |
| **Suspect base rate** | `render_probe` with `verdict=suspect` per healthy session, by platform — the input to the §2.3 widening decision (does the rebuild trigger stay `dead`-only, or do suspects cluster like the incident: iOS, round ≥2, HUD alive?). A `suspect` is instrumented but never acted on in v1, so this measures the blank-sample base rate on healthy canvases against genuine paint-death | `render_probe`, `pano_session` |
| **Recovery efficacy** | `render_recovery` with `result=recovered` ÷ `render_dead` issues — of the canvases we condemned, how many did the bounded in-place rebuild actually bring back? Complemented by the `rebuild_failed`/`still_dead` split (the GPU layer was simply gone) | `render_recovery`, `render_dead` |
| **iOS Daily completion convergence** | The product outcome: iOS Daily `daily_challenge_started`→`daily_challenge_completed` rate converging toward Android's (the ~47%→74% neighborhood the owner's wife is a datapoint in). The single number §18 exists to move | `daily_challenge_started`, `daily_challenge_completed` |
