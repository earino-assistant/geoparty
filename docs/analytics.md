...
| `modifier_callout_shown` | `mode, modifier, round_number` | guessing phone | Guess-modifier design §5.1: the pin-drop callout pill rendered — the top of the discovery funnel. `modifier` is which tease showed (`super` \| `decoy`, registry priority: SUPER first, decoy only when SUPER is spent). At most one per game per team by design (§3.4), and only from round 2 on (round 1 stays calm). Static copy only — no team name/room/place rides. `round_number` is the round the tease finally landed on |
| `modifier_sheet_opened` | `mode, modifier, via` | guessing phone | A modifier's sheet opened (the one place each rule is explained). `via` is the path in: `chip` (the action-bar chip tapped) \| `callout` (the pin-drop tease converted) \| `cross` (the other modifier's sheet cross-offered it). **Supersedes `super_sure_sheet_opened`** (removed from the schema): same funnel role, now per-modifier and per-path. The old event's 14 days of history stays queryable in PostHog. `mode` is `h2h` \| `couch` (the Daily has no bet). Aggregates only: the sheet opens while the player is still aiming, so nothing about the pin may ride along |
