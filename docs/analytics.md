...
| `round_started` | `room, mode, round_number, advance` | host phone | Round pushed live (once per round per game). `advance` (S6) is how the round was reached from the previous reveal: `auto` (the soft auto-advance countdown fired) \| `manual` (host tapped Next Round). Absent on round 1, which no reveal precedes |
